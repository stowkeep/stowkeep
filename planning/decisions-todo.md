...
| DEF-002 | Casbin policy authoring UX prototype | Stage 2 kickoff | Per D-010 mitigation. If the prototype is unusable, that's the trigger to revisit the engine choice. |
...
| OPEN-005 | **Source-available vs OSS posture during pre-alpha.** If contributors *do* show up, what's the policy — accept PRs, defer, archive issues? | Before first external interest | Avoids embarrassing back-and-forth with early contributors. |
